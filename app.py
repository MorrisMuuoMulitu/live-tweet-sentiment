...
try:
    ...
    st.warning(
        f"Twitter integration not available: {str(e)}. Using simulation mode. Ensure twitter_stream.py is in the project directory or add the required library to requirements.txt.")
...
class GroqSentimentAnalyzer:
    def __init__(self):
        ...
        print(
            f"Initializing sentiment analysis with {self.model_name} on Groq...")
    ...
    def _analyze_with_groq(self, text):
        ...
        try:
            # TODO: Replace with actual Groq API call per https://x.ai/api
            # Example:
    ...
    def _simulate_sentiment(self, text):
        ...
        positive_words = ['great', 'awesome', 'excellent',
                          'good', 'love', 'amazing', 'faster', 'incredible']
        negative_words = ['bad', 'terrible', 'awful', 'hate',
                          'disappointed', 'issues', 'error', 'frustrated']
...
class SlackAlerter:
    def __init__(self, webhook_url=None):
        self.webhook_url = webhook_url or os.environ.get(
            "SLACK_WEBHOOK_URL") or "https://hooks.slack.com/services/REPLACE/WITH/YOUR_WEBHOOK_URL"
    ...
    if 'tweet_source' not in st.session_state:
        if TWITTER_AVAILABLE:
            try:
                ...
                st.error(
                    f"Error initializing Twitter API: {str(e)}. Falling back to simulation.")
...
def update_word_counts(text):
    ...
    stop_words = {'the', 'a', 'an', 'and', 'is', 'in', 'it', 'to', 'with', 'for',
                  'of', 'on', 'at', 'by', 'from', 'up', 'about', 'into', 'over', 'after'}
    words = [
...
def main():
    ...
    st.markdown(
        f"Real-time sentiment analysis of tweets with **{st.session_state.hashtag}** hashtag")
    ...
    with st.sidebar:
        ...
        st.info(
            f"⚠️ Alert will be sent to Slack if 3 or more negative tweets are detected within 10 seconds")
    ...
    with col1:
        ...
        colors = {'Positive': '#4CAF50',
                  'Neutral': '#FFC107', 'Negative': '#F44336'}
    ...
    with col2:
        ...
        if st.session_state.tweets:
            ...
            for tweet in st.session_state.tweets[:10]:
